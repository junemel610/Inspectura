        ...
        if success:
            ...
            if hasattr(self, 'status_label'):
                if hasattr(self.status_label, 'configure'):
                    self.status_label.configure(text="Status: Cameras reassigned successfully", text_color="green")
        else:
            print("Runtime camera reassignment failed")
            if hasattr(self, 'status_label'):
                if hasattr(self.status_label, 'configure'):
                    self.status_label.configure(text="Status: Camera reassignment failed", text_color="red")
        return success
    ...
    def reassign_arduino_runtime(self):
        ...
        try:
            ...
            if self.ser and self.ser.is_open:
                ...
                if hasattr(self, 'status_label'):
                    if hasattr(self.status_label, 'configure'):
                        self.status_label.configure(text="Status: Arduino reassigned successfully", text_color="green")
                return True
            else:
                print("Runtime Arduino reassignment failed")
                if hasattr(self, 'status_label'):
                    if hasattr(self.status_label, 'configure'):
                        self.status_label.configure(text="Status: Arduino reassignment failed", text_color="red")
                return False
        except Exception as e:
            print(f"Error during runtime Arduino reassignment: {e}")
            if hasattr(self, 'status_label'):
                if hasattr(self.status_label, 'configure'):
                    self.status_label.configure(text="Status: Arduino reassignment error", text_color="red")
            return False
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Helper method for updating status label (CTkLabel version)
        def update_status_text(text, color=None):
            if hasattr(self.status_label, 'configure'):
                # CTkLabel for new GUI
        ...
        # Create GUI with new modern design
    ...
    def create_modern_gui(self):
        """Create the new modern GUI layout with tabbed interface"""
        screen_width = self.winfo_screenwidth()
        screen_height = self.winfo_screenheight()
        ...
        # Store canvas dimensions as instance attributes
        self.canvas_width = camera_width
        self.canvas_height = camera_height

        # Left Camera Feed (Top Camera)
        self.top_canvas = tk.Canvas(self.live_tab, width=camera_width, height=camera_height,
                                    bg='black', highlightbackground="#cccccc", highlightthickness=2)
        self.top_canvas.place(x=padding, y=padding)

        # Right Camera Feed (Bottom Camera)
        self.bottom_canvas = tk.Canvas(self.live_tab, width=camera_width, height=camera_height,
                                     bg='black', highlightbackground="#cccccc", highlightthickness=2)
        self.bottom_canvas.place(x=padding + camera_width + padding, y=padding)
        ...
        for i, title in enumerate(defect_titles):
            ...
            main_scroll = ctk.CTkScrollableFrame(defect_frame, width=defect_panel_width - 20,
                                                height=defect_panel_height - 100, fg_color="transparent")
            main_scroll.pack(fill="both", expand=True, padx=5, pady=(0, 5))
            
            # Container inside scrollable frame for two columns
            content_frame = ctk.CTkFrame(main_scroll, fg_color="transparent")
            content_frame.pack(fill="both", expand=True)
            
            # Configure grid for two columns
            content_frame.grid_columnconfigure(0, weight=1, uniform="defect_col")
            content_frame.grid_columnconfigure(1, weight=1, uniform="defect_col")

            # TOP column container
            top_container = ctk.CTkFrame(content_frame, fg_color="transparent")
            top_container.grid(row=0, column=0, sticky="nsew", padx=(0, 2))
            
            # BOTTOM column container
            bottom_container = ctk.CTkFrame(content_frame, fg_color="transparent")
            bottom_container.grid(row=0, column=1, sticky="nsew", padx=(2, 0))
            ...
            self.defect_labels[f"{title}_top"] = top_label
            self.defect_labels[f"{title}_bottom"] = bottom_label
            self.defect_scrolls[title] = main_scroll  # Store main scrollable frame
            self.defect_containers[f"{title}_top"] = top_container
            self.defect_containers[f"{title}_bottom"] = bottom_container
        ...
        # Keep old grade_label as an alias for combined_grade_label for compatibility
        self.grade_label = self.combined_grade_label
        
        # Create hidden individual camera grade labels (needed for backend but not displayed)
        self.top_grade_label = ctk.CTkLabel(grade_frame, text="No Grade", font=("Arial", 12, "bold"))
        self.bottom_grade_label = ctk.CTkLabel(grade_frame, text="No Grade", font=("Arial", 12, "bold"))
        # Don't pack these labels - they exist only for backend compatibility
        ...
        # Create close button at top right (after all other widgets)
        self.close_button = ctk.CTkButton(
            self, text="✕", command=self.on_closing,
            width=40, height=40, corner_radius=6,
            fg_color="#dc3545", hover_color="#c82333",
            font=("Arial", 18, "bold")
        )
        self.close_button.place(relx=1.0, x=-50, y=10, anchor="ne")
        self.close_button.lift()  # Bring to front

    def update_live_grading_display(self):
        # Don't reset grades in IDLE mode - keep last graded value visible
    ...
    def clear_defect_displays(self):
        ...
        print("🧹 Cleared all defect displays for new wood piece")

    def add_defect_to_display(self, defect_type, image_path, size_mm, camera_name):
        """Add a defect image to the appropriate panel
        
        Args:
            defect_type: Type of defect (e.g., "DeadKnot", "SoundKnot", "MissingKnot", "KnotWithCrack")
            image_path: Path to the saved defect image
            size_mm: Size of defect in millimeters
            camera_name: "top" or "bottom"
        """
        # Map defect types to panel titles (handles both underscore and no-underscore versions)
        defect_to_panel = {
        ...
        panel_title = defect_to_panel.get(defect_type)
        if not panel_title:
            print(f"⚠️  Unknown defect type: {defect_type}")
            return
        
        # Create key with camera name (e.g., "DEAD KNOTS_top")
        container_key = f"{panel_title}_{camera_name}"
        
        if container_key not in self.defect_containers:
            print(f"⚠️  Container not found: {container_key}")
            return
        
        container = self.defect_containers[container_key]
        
        # Remove "No detected items" label if it exists
        if container_key in self.defect_labels:
            old_label = self.defect_labels[container_key]
            if old_label is not None and old_label.winfo_exists() and old_label.cget("text") == "No detected items":
                old_label.destroy()
                self.defect_labels[container_key] = None
        ...
        try:
            ...
            print(f"❌ Failed to add defect image to display: {e}")
            import traceback
            traceback.print_exc()
            ...
            if current_time > self._camera_check_cooldown and not in_grace_period:
                ...
                if not camera_status['both_ok']:
                    ...
                    if self.camera_handler.reassign_cameras_runtime():
                        ...
                        self.camera_disconnected_popup_shown = False
                        if hasattr(self, 'status_label'):
                            # status_label is a CTkLabel widget
                            self.status_label.configure(text="Status: Cameras reconnected", text_color="green")
                    else:
                        print("Camera reconnection failed during runtime")
                        if hasattr(self, 'status_label'):
                            # status_label is a CTkLabel widget
                            self.status_label.configure(text="Status: Camera reconnection failed", text_color="red")
    ...
    def update_detection_status_display(self):
        ...
        if hasattr(self, 'status_label'):
            # Helper function to update CTkLabel properly
            def update_status_text(text, foreground="black"):
                self.status_label.configure(text=text, text_color=foreground)
    ...
    def start_automatic_detection(self):
        ...
        # Clear defect display panels for new wood piece (via message queue for thread safety)
        self.message_queue.put(("clear_defects",))
    ...
    def save_defect_crop(self, frame, bbox, defect_class, size_mm, camera_name):
        ...
        try:
            ...
            # Add to GUI display
            self.add_defect_to_display(defect_class_formatted, filepath, size_mm, camera_name)
    ...
    def process_message_queue(self):
        ...
        try:
            while True:
                ...
                if msg_type == "clear_defects":
                    # Clear defect displays from main thread
                    self.clear_defect_displays()
                    continue
                ...
                if msg_type == "arduino_message":
                    ...
                    if message == "B":
                        ...
                        if self.current_mode == "TRIGGER":
                            if not self.auto_detection_active:
                                # Clear defect displays for new wood piece
                                self.message_queue.put(("clear_defects",))
                                ...
                                if hasattr(self, 'status_label'):
                                    # status_label is a CTkLabel widget
                                    self.status_label.configure(text="Status: IR TRIGGERED - Motor should be running!", text_color="orange")
                            ...
                            if not self.scan_phase_active:
                                ...
                                if hasattr(self, 'status_label') and hasattr(self.status_label, 'configure'):
                                    self.status_label.configure(text="Status: SCAN_PHASE STARTED - Scanning segments...", text_color="orange")
                        ...
                        try:
                            ...
                            if self.current_mode == "TRIGGER":
                                ...
                                if self.auto_detection_active:
                                    if hasattr(self, 'status_label'):
                                        # status_label is a CTkLabel widget
                                        self.status_label.configure(text="Status: Processing results...", text_color="red")
                                    self.stop_automatic_detection_and_grade()
                                    if hasattr(self, 'status_label'):
                                        # status_label is a CTkLabel widget
                                        self.status_label.configure(text="Status: Ready - Waiting for IR beam trigger", text_color="green")
                                else:
                    ...
                    # --- OTHER ARDUINO MESSAGES ---
                        ...
                        # Check if we're in camera reconnection grace period and suppress old error messages
                        in_grace_period = False
                        if hasattr(self, 'camera_reconnection_grace_start'):
                            grace_elapsed = time.time() - self.camera_reconnection_grace_start
                            if grace_elapsed < self.camera_reconnection_grace_period:
                                in_grace_period = True
                                # Suppress old error messages during grace period
                                if any(error_keyword in message for error_keyword in 
                                      ["ERROR_RECEIVED", "SYSTEM_PAUSED", "CAMERA_DISCONNECTED"]):
                                    print(f"🛡️ Grace period: Suppressing old Arduino error message: '{message}'")
                                    continue
                        ...
                        if message.startswith("STATUS_PAUSED:"):
                            ...
                            if hasattr(self, 'status_label'):
                                self.status_label.configure(text=f"Status: Arduino: {message}")
                        else:
    ...
    def send_arduino_command(self, command):
        ...
        try:
            if self.ser:
                ...
                if hasattr(self, 'status_label'):
                    # status_label is a CTkLabel widget
                    self.status_label.configure(text="Status: Arduino not connected.")
            ...
            if hasattr(self, 'status_label'):
                # status_label is a CTkLabel widget
                self.status_label.configure(text="Status: Arduino communication error - reconnection in progress...")
    ...
    def set_idle_mode(self):
        ...
        self.wood_detection_results = {"top": None, "bottom": None}
        self.dynamic_roi = {}
        # NOTE: Grade label is NOT reset here - it retains the last graded wood value
        # DON'T reset grades to empty when entering idle mode - keep last values visible
        # self.live_grades = {"top": "", "bottom": ""}
        # self.update_live_grading_display()
        ...
        # Update status label for CTkLabel
        if hasattr(self.status_label, 'configure'):
    ...
    def create_session_folder(self):
        ...
        # Use absolute path to match base_detections_path
        folder = os.path.join(self.base_detections_path, timestamp)
        ...
        with open(os.path.join(wood_folder, "defects.txt"), "w") as f:
            ...
            if data['top_defects']:
                for i, (defect_type, size_mm, percentage) in enumerate(data['top_defects'], 1):
                    ...
                    defect_line = f"{i}. {display_name} - {round(size_mm, 2)}mm\n"
                    f.write(defect_line)
    ...
    def update_wood_counter_display(self):
        ...
        if hasattr(self, 'wood_counter_label'):
            ...
            self.wood_counter_label.configure(text=f"🪵 Current Wood: #{wood_num}")
    ...
    def finalize_grading(self, final_grade, all_measurements):
        ...
        # Update Reports tab grade counters (G2-0, G2-1, G2-2, etc.)
        grade_key = f"G2-{stat_index-1}"
        if hasattr(self, 'count_labels') and grade_key in self.count_labels:
            self.count_labels[grade_key].configure(text=str(self.grade_counts[stat_index]))
    ...
    def _execute_manual_grade(self):
        ...
        if wood_detected:
            ...
            # status_label is a CTkLabel widget
            self.status_label.configure(text="Status: Manual grade - no wood detected")
        ...
        try:
            ...
            if severity in ["CRITICAL", "ERROR"]:
                ...
                if error_type not in auto_recoverable_errors:
                    ...
                    if not hasattr(self, '_pending_desktop_notifications'):
                        self._pending_desktop_notifications = {}
                    self._pending_desktop_notifications[error_type] = {
                        'time': current_time,
                        'details': details,
                        'severity': severity
                    }
    ...
    def clear_error(self, error_type):
        ...
        try:
            ...
            # Cancel pending desktop notification for this error (auto-recovery successful)
            if hasattr(self, '_pending_desktop_notifications'):
                if error_type in self._pending_desktop_notifications:
                    self._pending_desktop_notifications.pop(error_type, None)
                    print(f"🛡️ Cancelled pending desktop notification for {error_type}")
        ...
        try:
            ...
            if hasattr(self, 'camera_reconnection_grace_start'):
                ...
                if grace_elapsed < self.camera_reconnection_grace_period:
                    ...
                    # Grace period expired - remove grace period attributes and set buffer
                    delattr(self, 'camera_reconnection_grace_start')
                    if hasattr(self, '_grace_period_logged'):
                        delattr(self, '_grace_period_logged')
                    self._grace_period_just_expired = time.time()  # Mark when grace expired for buffer period
                    print("🛡️ Camera reconnection grace period expired - starting 30s stabilization buffer")
            ...
            if not camera_status['both_ok']:
                ...
                # Check if we just exited grace period - allow additional stabilization time
                if hasattr(self, '_grace_period_just_expired'):
                    elapsed_since_grace = time.time() - self._grace_period_just_expired
                    if elapsed_since_grace < 30.0:  # Additional 30 seconds buffer
                        print(f"⏳ Camera still stabilizing after grace period ({30.0 - elapsed_since_grace:.1f}s buffer remaining) - not registering error")
                        print("   Will retry camera initialization...")
                        # Try to re-initialize cameras
                        try:
                            if not camera_status['bottom_ok'] and hasattr(self, 'cap_bottom'):
                                print("   Attempting to refresh bottom camera connection...")
                                self.cap_bottom = self.camera_handler.bottom_camera
                        except:
                            pass
                        return  # Skip error registration during buffer period
                    else:
    ...
    def show_error_alert(self, error_type, message, severity="WARNING"):
        ...
        try:
            ...
            # Desktop notification is now handled via delayed mechanism (not here)
            # See _send_delayed_desktop_notification() which is called 3 seconds after error registration
            
        except Exception as e:
            print(f"Error showing alert: {e}")
    
            
    def _send_delayed_desktop_notification(self, error_type):
        """
        Send desktop notification for an error - but only if error still exists.
        This is called 3 seconds after error registration to allow auto-recovery
        (like camera reconnection) to clear the error before notifying the user.
        """
        try:
            ...
            if error_type not in self._pending_desktop_notifications:
                print(f"✅ Desktop notification cancelled - {error_type} was auto-recovered")
                return
            
            # Check if error was cleared
            if error_type not in self.error_state["active_errors"]:
                print(f"✅ Desktop notification cancelled - {error_type} was cleared")
                # Remove from pending
                self._pending_desktop_notifications.pop(error_type, None)
                return
            
            # Error still exists - send notification
            pending = self._pending_desktop_notifications[error_type]
            ...
            error_name = error_config.get("name", error_type)
            title = f"CRITICAL SYSTEM ERROR" if severity == "CRITICAL" else f"System {severity.capitalize()}"
            ...
            # Remove from pending
            self._pending_desktop_notifications.pop(error_type, None)
            print(f"📢 Desktop notification sent for {error_type} (error persisted)")
            
        except Exception as e:
            print(f"Error in delayed desktop notification: {e}")
    ...
    def show_toast_notification(self, title, message, duration=5000, type="info"):
        ...
        try:
            ...
            toast_width = 450
            toast_height = 185  # Increased from 100 to accommodate timestamp
